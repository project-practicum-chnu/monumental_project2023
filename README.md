...
1. Внутрішній сервіс перевіряє присутність товару у БД. Якщо будь-яке з полів є недійсним, повертається відповідь у форматі JSON зі статусом "помилка".
2. Перевірка на дублювання. Бекенд перевірить, чи такий товар вже є в корзині. Якщо це так, товар не буде доданий буде повернуто повідомлення про помилку про наявність товару у кошику.
3. Якщо перевірка пройшла успішно і дублікатів не знайдено, до кошика буде додано товар. Відповідь у форматі JSON зі статусом "success" буде надіслано назад на фронт-енд.
4. Після успішного додавання HTMX на стороні клієнта проходить оновлення відображення вмісту кошика.
...
Для функціоналу додавання товару до кошика необхідно надати кнопці "Додати до кошика" атрибут `hx-post`, який відправляє асинхронний HTTP POST-запит для додавання товару до кошика без перезавантаження сторінки. Якщо потрібно використовувати додаткове поле з кількістю бажаного товару, то зручніше буде використати форму, подібну до вказаних вище в цій документації (наприклад в розділі "Сторінка реєстрації")
...
HTMX використовується для асинхронної обробки відправлення форми. Коли змінюється кількість одиниць товару або видалення товару HTTP POST-запит надсилається на кінцеву точку /api/cart/ без необхідності повного перезавантаження сторінки. Це покращує взаємодію з користувачем.
Для оформлення замовлення кнопка "Оформити замовлення" відправляє асинхронний HTTP POST-запит надсилається на кінцеву точку /api/checkout/.
...
Ця сторінка відображає зроблене замовлення і включає відображення замовлених товарів, обраних варіантів відправки та оплати та адресу одержувача.

Початкове завантаження сторінки

При переході на сторінку покупки користувач бачить список замовлених товарів у вигляді:
- "Зображення товару": Зображення товару, що допомагає користувачу візуально ідентифікувати товар.
- "Назва товару": Назва товару, яка дозволяє користувачу легко ідентифікувати товар.
- "Ціна": Ціна товару за одиницю.
- "Кількість": кількість замовлених одиниць кожного товару

Обробка замовлення

Після додавання товарів до корзини користувач має можливість перейти до процесу оформлення замовлення. Користувач може ввести адресу доставки та інші необхідні дані для оформлення покупки.

Оплата замовлення

Після введення інформації про замовлення користувач може обрати метод оплати та здійснити оплату свого замовлення. Система обробки платежів гарантує безпеку операції.

HTML структура

HTML-розмітка сторінки "Сторінка покупки" містить такі елементи:
```html
<!DOCTYPE html>
<html>
<head>
 <title>Сторінка покупки</title>
 <link rel="stylesheet" href="css/order.css">
</head>
<body>
 <div id="order-container">
   <h1>Сторінка покупки</h1>
   <ul>
     <li>
       <img src="product1.webp" alt="Назва товару 1">
       <h2>Назва товару 1</h2>
       <p>Ціна: 10.00 грн</p>
     </li>
     <!-- Інші товари -->
   </ul>
...
Стилістика сторінки "Сторінка покупки" знаходиться в окремому CSS-файлі і може бути налаштована відповідно до дизайну вашого веб-застосунку. Нижче наведений приклад стилів для елементів на сторінці:
```css
#shopping-container {
 width: 80%;
 margin: auto;
...
![image](https://github.com/project-practicum-chnu/monumental_project2023/assets/75749649/1f7c9d42-faad-4b01-9df1-b1779aa2ad19)
